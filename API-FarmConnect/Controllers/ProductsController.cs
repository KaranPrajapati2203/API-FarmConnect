        ...
        public async Task<IActionResult> AddProductListing([FromBody] ProductListingRequest request)
        {
            try
            {
                using (var connection = new NpgsqlConnection(_connectionString))
                {
                    ...
                    var insertListingSql = "INSERT INTO ProductListings (SellerId, ProductId, ListingQuantity, ProductMeasureType) " +
                                   "VALUES (@SellerId, @ProductId, @ListingQuantity, @ProductMeasureType) RETURNING ListingId";
                    long newListingId;
                    using (var cmd = new NpgsqlCommand(insertListingSql, connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@ProductMeasureType", request.ProductMeasureType);
